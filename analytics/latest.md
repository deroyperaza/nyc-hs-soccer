# Traffic

_2026-09-18T16:27:20Z, last 30 days. Cloudflare Web Analytics, cookieless._

| | visits | pageviews |
|---|---:|---:|
| today | 140 | 630 | 
| yesterday | 310 | 920 |
| 30 days | 450 | 1550 |

Pageviews run roughly double: the app rewrites its own URL as you move
around, and each rewrite is a beacon. Visits counts one per arrival, so
that is the number that means something.

> Note: CF_SITE_TAG matched no site with traffic, and the account has exactly one that does, so this report uses it. The stored value is almost certainly the beacon token from the page rather than the site tag -- different ids.

## By day

| date | visits | pageviews |
|---|---:|---:|
| 2026-09-17 | 310 | 920 |
| 2026-09-18 | 140 | 630 |

## Most-read pages

| path | views | share |
|---|---:|---:|
| / | 520 | 34% |
| /game/502587/boys | 80 | 5% |
| /teams/boys | 70 | 5% |
| /players/boys | 70 | 5% |
| /scores/2026-09-20/boys | 40 | 3% |
| /school/murrow/boys | 40 | 3% |
| /school/graphics/boys | 40 | 3% |
| /standings/boys | 30 | 2% |
| /school/martin-l-king/boys | 30 | 2% |
| /scores/2026-09-16/boys | 30 | 2% |
| /player/zohran-mamdani-bronx-science/boys | 30 | 2% |
| /scores/2026-09-12/boys | 20 | 1% |
| /school/beacon/2026/boys | 20 | 1% |
| /scores/2026-09-19/boys | 20 | 1% |
| /school/lehman/boys | 20 | 1% |
| /player/cyrus-vossoughian-bard-manhattan/boys | 20 | 1% |
| /game/502547/boys | 20 | 1% |
| /scores/2026-09-22/boys | 20 | 1% |
| /scores/2026-09-17/boys | 20 | 1% |
| /history | 20 | 1% |

## Where people came from

| referrer | views | share |
|---|---:|---:|
| nychssoccer.com | 1100 | 71% |
| (direct) | 440 | 28% |
| www.psal.org | 10 | 1% |

## Countries

| country | views | share |
|---|---:|---:|
| US | 1550 | 100% |

## Devices

| device | views | share |
|---|---:|---:|
| mobile | 1280 | 83% |
| desktop | 270 | 17% |

